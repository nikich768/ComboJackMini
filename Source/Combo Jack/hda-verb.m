...
int main(void)
{
    ...
    if (sem_open("ComboJackNano_Watcher", O_CREAT, 600, 1) == SEM_FAILED)
    {
    ...
    while(run) // Poll patcher
    {
        ...
        usleep(3000); // Sleep delay (microseconds): use sleep() instead of usleep() for seconds, but i don't think you need it
    }

    sem_unlink("ComboJackNano_Watcher");
